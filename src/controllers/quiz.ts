  ...
  switch (question.type) {
    ...
    case "MULTIPLE_SELECT":
      return (
        ...
        question.correctAnswer.every((idx: any) => 
    ...
    case "FILL_IN_BLANK":
      return (
        Array.isArray(question.correctAnswer) &&
        question.correctAnswer.length > 0 &&
        question.correctAnswer.every((answer: any) => 
    ...
    switch (question.type) {
      ...
      case "MULTIPLE_SELECT":
        if (
          ...
          const correctCount = userAnswers.filter((a: number) =>
      ...
      case "FILL_IN_BLANK":
        if (Array.isArray(question.correctAnswer)) {
          ...
          const correctAnswers = question.correctAnswer.map((ans: string) => 
...
export const createQuiz = async (req: AuthRequest, res: Response) => {
  try {
    const { title, description, duration, isPublic, password, questions } =
      req.body as CreateQuizRequest;
...
export const updateQuiz = async (
  ...
  try {
    const { id } = req.params;
    const { title, description, duration, isPublic, password, imagesToDelete } =
      req.body as UpdateQuizRequest & { imagesToDelete?: string[] };
    ...
    if (imagesToDelete && imagesToDelete.length > 0) {
      const imagesToRemove = existingQuiz.images.filter((img) =>
...
export const updateQuestion = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const updatedQuestion = await prisma.question.update({
      ...
      data: {
        ...updates,
        correctAnswer: updates.correctAnswer !== undefined ? updates.correctAnswer as any : undefined,
      },
      include: {
        images: true, // Include question images
      },
...
export const searchQuizzes = async (req: Request, res: Response) => {
  try {
    ...
    const orderBy: Prisma.QuizOrderByWithRelationInput = {
      [sortBy as keyof Prisma.QuizOrderByWithRelationInput]: sortOrder as
        | "asc"
        | "desc",
    };
...
export const getQuizzes = async (req: AuthRequest, res: Response) => {
  try {
    const quizzes = await prisma.quiz.findMany({
      where: {
        OR: [{ isPublic: true }, { teacherId: req.user?.id }],
      },
      include: {
        teacher: {
          select: {
...
export const getResponseDetails = async (req: AuthRequest, res: Response) => {
  try {
    ...
    response.map((e) => {
      if (e.quiz.teacherId == req.user?.id || e.userId == req.user?.id) {
        responseDetails.push({
          ...
          questions: e.quiz.questions.map((question) => {
            const userAnswers = e.answers as unknown as QuizAnswer[];
            ...
            switch (question.type) {
              ...
              case "INTEGER":
                isCorrect =
                  Number(userAnswer?.answer) === question.correctAnswer;
                obtainedMarks = isCorrect ? question.marks : 0;
                break;
            }
